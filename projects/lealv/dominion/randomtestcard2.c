...
int main() {
	...
	for (n = 0; n < NUM_TESTS; n++)
	{
		...
		G.numActions = floor(Random() * 256); //0 to 255 actions to start.
		...
		{
			printf("FAILURE: Test #%d: numActions = %d, deckCount = %d / discardCount = %d / handCount = %d / playedCardCount = %d\n\n", n + 1, G.numActions, G.deckCount[player], G.discardCount[player], G.handCount[player], G.playedCardCount);
		}
	}

	//------Additional tests.--------
	int numAdditionalTests = 1;
	//Hand, discard, deck and played card count and numActions are all 0.
	int i;
	for (i = 0; i < sizeof(struct gameState); i++)
	{
		((char*)&G)[i] = floor(Random() * 256);

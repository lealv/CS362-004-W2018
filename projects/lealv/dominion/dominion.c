...
void adventurer_effect(struct gameState *state, int handPos)
{
	int drawntreasure = 0;
	int currentPlayer = whoseTurn(state);
	int cardDrawn;
	...
	while (z - 1 >= 1) { //BUG: Changed (z - 1 >= 0) to (z - 1 >= 1).
...
void smithy_effect(struct gameState *state, int handPos)
{
	...
	//+3 Cards
	int i;
	for (i = 1; i < 3; i++) //BUG changed i=0 to i=1.
	{
		drawCard(currentPlayer, state);
	}
...
void village_effect(struct gameState *state, int handPos)
{
	...
	//+2 Actions
	state->numActions = 2; //BUG changed [= state->numActions + 2] to [= 2].

	//discard played card from hand
	discardCard(handPos, currentPlayer, state, 0);
}

void great_hall_effect(struct gameState *state, int handPos)
{
	int currentPlayer = whoseTurn(state);
...
void steward_effect(struct gameState *state, int handPos, int choice1, int choice2, int choice3)
{
	int currentPlayer = whoseTurn(state);
	...
	else
	{
		...
		discardCard(choice1, currentPlayer, state, 1); //BUG: changed choice2 to choice1.
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
		adventurer_effect(state, handPos);
		return 0;
    ...
    case smithy:
		smithy_effect(state, handPos);
		return 0;
		
    case village:
		village_effect(state, handPos);
		return 0;
    ...
    case great_hall:
		great_hall_effect(state, handPos);
		return 0;
    ...
    case steward:
		steward_effect(state, handPos, choice1, choice2, choice3);
		return 0;
